...
class StringCleaner {
	constructor(str) {
		if(typeof str === 'undefined' || !str.toString) {
			throw new Error('StringCleaner constructor must recieve a value with a toString() method')
		}
		this.s = str.toString();
	}
	...
	removeChars(options) {
		...
		const re = new RegExp(`[^a-z\\s${ escapeStringRegexp(options.exclude) } ]`, 'gi');
		return this.replace(re, options.replaceWith);
	}
	...
	removeApostrophies() {
		...
		while ((match = re.exec(this.s)) !== null) {
			this.s = this.s.substring(0, match.index + 1) + this.s.substring(match.index + 2, this.s.length)
		}
	...
	removeDashes() {
		...
		while ((match = re.exec(this.s)) !== null) {
			this.s = this.s.substring(0, match.index) + match[0].replace(/-/g, '') + this.s.substring(match.index + match[0].length, this.s.length)
		}
